...
```{r}
gisaid_india %>% filter(pangolin_lineage %in% c("BA.4", "BA.5")) %>% pull(State) %>% table() %>% as.data.frame() %>% arrange(desc(Freq)) %>% DT::datatable()
...
```{r}
gisaid_india %>% filter(pangolin_lineage %in% c("BA.4", "BA.5")) %>% pull(custom_city) %>% table() %>% as.data.frame() %>% arrange(desc(Freq)) %>% DT::datatable()
```
